...
export default function UserStep1({ user, step1Data }) {
    const isEditing = !!user;
    
    const { data, setData, post, put, errors, processing } = useForm({
        title: step1Data?.title || user?.title || '',
        user_Fname: step1Data?.user_Fname || user?.user_Fname || '',
        user_Lname: step1Data?.user_Lname || user?.user_Lname || '',
        user_email: step1Data?.user_email || user?.user_email || '',
        user_phone: step1Data?.user_phone || user?.user_phone || '',
        gender: step1Data?.gender || user?.gender || 'male',
        userType: step1Data?.userType || user?.userType || 'instructor',
        profession: step1Data?.profession || user?.profession || '',
        custom_url: step1Data?.custom_url || user?.custom_url || '',
        specialities: step1Data?.specialities || user?.specialities || '',
        userLocality: step1Data?.userLocality || user?.userLocality || '',
        userCountry: step1Data?.userCountry || user?.userCountry || '',
        userState: step1Data?.userState || user?.userState || '',
        elivatorPitch: step1Data?.elivatorPitch || user?.elivatorPitch || '',
        user_img: step1Data?.user_img || user?.user_img || '',
    });

    const submit = (e) => {
        e.preventDefault();
        post(route('users.store.step1'));
    };

    return (
        <AuthenticatedLayout>
            <Head title={isEditing ? "Edit Instructor" : "Create Instructor - Step 1"} />
                        ...
                        <div className="px-6 py-6">
                            <div className="mb-6 flex justify-between items-center">
                                <h2 className="mt-2 text-2xl font-bold text-gray-900">
                                    {isEditing ? "Edit Instructor" : "Create New Instructor - Step 1 of 3"}
                                </h2>
                                {!isEditing && (
                                    <Link 
